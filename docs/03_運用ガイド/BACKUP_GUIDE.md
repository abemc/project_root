...
2. **サイドバーの「🔧 ストレージ設定」を展開**
   - 左サイドバー → 「💾 バックアップ・リストア」セクション
   - 「🔧 ストレージ設定」エクスパンダーをクリック

3. **外部ドライブを有効化**
   - 「☑️ 外部ドライブを使用」にチェック
   - ドライブ種別を選択:
     - **Linux/WSL パス**: `/mnt/external_drive` など直接パス入力
     - **Windows ドライブ**: ドロップダウンから D, E, F... を選択（WSL 環境で自動変換）
     - **macOS パス**: `/Volumes/External` などを入力

4. **バックアップ対象を選択**
   - 📦 作成 タブ
   - 対象を選択（デフォルト：system_config, source_code, documentation）
   - ⚠️ embeddings, models, knowledge_base は大容量のため個別確認を推奨

5. **オプション設定**
   - 「☑️ 圧縮を有効」：圧縮する場合にチェック
   - 「☑️ メモを追加」：バックアップ説明を追加する場合にチェック

6. **「✨ バックアップを作成」をクリック**
   - 進行状況を表示
   - 完了後、「📋 バックアップ詳細情報」で保存先を確認
...
```
1. 「☑️ 外部ドライブを使用」にチェック
2. ドライブ種別：「Windows ドライブ」を選択
3. ドライブ選択：「D」を選択
4. 自動的に /mnt/d に変換され、✅ マークが表示される
5. バックアップ先: /mnt/d/backups が使用される
```

**Linux で USB ドライブを指定**
```
1. 「☑️ 外部ドライブを使用」にチェック
2. ドライブ種別：「Linux/WSL パス」を選択
3. パス欄に入力: /mnt/usb_drive
4. ✅ パスが確認されるか ⚠️ 警告が表示される
5. バックアップ先: /mnt/usb_drive/backups が使用される
```

**macOS で外部ドライブを指定**
```
1. 「☑️ 外部ドライブを使用」にチェック
2. ドライブ種別：「macOS パス」を選択
3. パス欄に入力: /Volumes/External
4. ✅ パスが確認されるか ⚠️ マウント確認の指示が表示される
5. バックアップ先: /Volumes/External/backups が使用される
```
